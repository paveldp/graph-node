...
use graph::{components::store::StoredDynamicDataSource, prelude::SubgraphDeploymentEntity};
...
use graph::data::subgraph::schema::{POI_OBJECT, SUBGRAPHS_ID};
use graph::prelude::{
...
impl Store {
    ...
    async fn with_entity_conn<T: Send + 'static>(
        ...
        self.with_conn(move |conn, cancel_handle| {
            ...
            cancel_handle.check_cancel()?;
            let layout = store
                .layout(&conn, &site.namespace, &site.deployment)
                .map_err(Into::<Error>::into)?;
            cancel_handle.check_cancel()?;
